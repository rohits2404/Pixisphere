...
import { FiCamera, FiMapPin, FiDollarSign, FiStar, FiArrowRight } from 'react-icons/fi'

const PhotographerCard = ({ p }: { p: Photographer }) => {
    return(
        <div className="group relative bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-blue-500">
            ...
            <div className="p-5">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                    {p.tags.map((tag,i) => (
                        <span
                        key={i}
                        className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-3 py-1 rounded-full text-xs font-medium shadow-sm"
                        >
                            {tag}
                        </span>
                    ))}
                ...
                <Link href={`/photographer/${p.id}`}>
                    <button className="mt-4 w-full flex items-center justify-center space-x-2 bg-gradient-to-r fom-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white py-3 px-4 rounded-lg font-medium transition-all duration-300 shadow-md hover:shadow-lg">
